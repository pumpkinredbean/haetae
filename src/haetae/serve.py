...
def systemone(req: Req):
    ...
    for name, q in req.questions.items():
        opts, names = _options(q)
        rec = {"state": state, "instructions": q.instructions, "options": opts}
        b = collate(_tok, [rec])
        with torch.no_grad():
            logits = _model(b["input_ids"].to(_device), b["attention_mask"].to(_device),
                            b["option_pos"].to(_device), b["group_ptr"].to(_device))[0]
